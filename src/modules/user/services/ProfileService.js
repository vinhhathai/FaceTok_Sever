...
class ProfileService {
  ...
  async updateProfile(userId, profileData) {
    try {
      console.log(`[ProfileService] Bắt đầu cập nhật profile cho userId: ${userId}`);
      ...
      if (!existingUser) {
        console.log(`[ProfileService] Không tìm thấy người dùng với ID: ${userId}`);
        return ProfileDto.error(
          errorCode.DATA_NOT_FOUND,
          errorMessage.USER_NOT_FOUND
        );
      ...
      console.log(`[ProfileService] Tìm thấy người dùng: ${existingUser.fullName}`);
      ...
      if (profileData.birthday !== undefined) updateData.birthday = new Date(profileData.birthday);
      ...
      if (!updatedUser) {
        console.log(`[ProfileService] Không thể cập nhật profile cho userId: ${userId}`);
        return ProfileDto.error(
      ...
      console.log(`[ProfileService] Cập nhật profile thành công cho userId: ${userId}`);

      // Format dữ liệu trả về bằng DTO
      const responseData = ProfileDto.toResponse(updatedUser);

      // Trả về kết quả thành công
      return ProfileDto.success(responseData, "Cập nhật thông tin cá nhân thành công");
    } catch (error) {
